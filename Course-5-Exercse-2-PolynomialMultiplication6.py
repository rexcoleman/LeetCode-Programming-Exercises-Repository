...
def multiply(A, B):
    ...
    for i in range(len(C)):
        # Determine the start and end indices for slices in A and B_flipped
        start_A = max(0, i - nb + 1)
        end_A = min(na, i + 1)
        start_B = max(0, nb - 1 - i)
        end_B = min(nb, nb + na - 1 - i)

        # Calculate the dot product of relevant slices
        C[i] = A[start_A:end_A] @ B_flipped[start_B:end_B]
