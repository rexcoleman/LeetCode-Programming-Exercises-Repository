...
class Solution:
    def longestZigZag(self, root: Optional[TreeNode]) -> int:
        def dfs(node, count, direction):
            nonlocal max_zig_zag_count
            count += 1
            max_zig_zag_count = max(max_zig_zag_count, count)
            if node.left:
                dfs(node.left, 0 if direction == 1 else count, 1)
            if node.right:
                dfs(node.right, 0 if direction == 0 else count, 0)

        max_zig_zag_count = 0
        dfs(root, -1, 1) if root else 0
        return max_zig_zag_count
