...
class Solution:
    def pathSum(self, root: Optional[TreeNode], targetSum: int) -> int:
        ...
        def dfs(node, list_above):
            nonlocal num_paths
            list_above.append(node.val)
            sum_list_above = sum(list_above)
            for i in range(len(list_above)):
                if sum_list_above == targetSum:
                    num_paths += 1
                sum_list_above -= list_above[i]
            if node.left:
                dfs(node.left, list_above)
            if node.right:
                dfs(node.right, list_above)
            list_above.pop()
